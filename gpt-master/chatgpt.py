...
while True:
    ...
    if not query:
        # Input the HTML and OCR-extracted text as multiple lines
        # html_input = get_multiline_input("HTML (input each line followed by Enter, and type ':wq' on a new line to finish):\n")
        url_input = input("url plz: ")
        query = f"{prompt}URL:\n{url_input}\n"
        # Combine the prompt with the user inputs
        # query = f"{prompt}HTML:\n{html_input}\n"
